...
import_customers_task = BashOperator(
    ...
    bash_command=f"""psql {AIRFLOW_CONN_SALES_OLTP} -c "\copy customers to stdout" | psql {AIRFLOW_CONN_SALES_DW} -c "\copy import.customers(customer_id, first_name, email, last_name) from stdin" """,
    dag=dag,
)
